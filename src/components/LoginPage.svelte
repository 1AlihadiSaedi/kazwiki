<script>
  ...
  async function handleLogin(e) {
    ...
    loading = true;
    try {
      const result = await signIn(username, name, password);
    ...
    <form class="login-form" onsubmit={handleLogin}>
      <div class="form-group">
        ...
        <input id="login-user" type="text" class="form-input" autocomplete="username"
          placeholder="root" bind:value={username} required dir="ltr" />
      </div>
      <div class="form-group">
        <label class="form-label" for="login-name">{t(lang, 'secretName')}</label>
        <input id="login-name" type="text" class="form-input" autocomplete="off"
          placeholder="••••••" bind:value={name} required dir="ltr" />
      </div>
      <div class="form-group">
...
<style>
  ...
  .login-card { width:100%; max-width:420px; background:var(--color-bg-primary); border:1px solid var(--color-border); border-radius:var(--radius-lg); padding:2.5rem 2.5rem; box-shadow:var(--shadow-lg); animation:fadeUp 350ms ease; }
