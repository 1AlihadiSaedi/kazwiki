...
{#if step === 1}
<div class="sfg"><label class="sfl">{t('adminUser')}</label><input class="sfi" type="text" bind:value={form.username} dir="ltr" /></div>
<div class="sfg"><label class="sfl">{t('adminPass')}</label><input class="sfi" type="password" bind:value={form.password} /></div>
<div class="sfg"><label class="sfl">{t('adminName')}</label><input class="sfi" type="text" bind:value={form.displayName} /></div>
{:else if step === 2}
<div class="sfg"><label class="sfl">{t('defaultLang')}</label><select class="sfi" bind:value={form.defaultLanguage}><option value="fa">Persian (fa)</option><option value="en">English (en)</option></select></div>
<div class="sfg"><label class="sfl">{t('extraLangs')}</label><div class="sad"><input class="sfi swi" type="text" placeholder="code" bind:value={newLangCode} /><input class="sfi swi" type="text" placeholder="name" bind:value={newLangName} /><button class="sab" onclick={addExtraLang}>+ {t('addLang')}</button></div>{#if form.extraLanguages.length > 0}<div class="sel">{#each form.extraLanguages as el}<span class="set">{el.code} - {el.name} <button class="sex" onclick={() => removeExtraLang(el.code)}>x</button></span>{/each}</div>{/if}</div>
{:else}
