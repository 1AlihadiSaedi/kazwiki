<script>
  ...
  function rr(){
    ...
    const cl=p.replace(/^\//,'');const segs=cl.split('/');
    let lfp=null;
    if(segs[0]==='edit'){cr='wiki';em=true;if(segs.length>=2&&LANGUAGES.includes(segs[1])){lfp=segs[1];cs=segs.slice(2).join('/')||HOME_PAGE}else{cs=segs.slice(1).join('/')||HOME_PAGE}}
    else if(LANGUAGES.includes(segs[0])){lfp=segs[0];if(segs[1]==='login'){cr='login';em=false}else if(segs[1]==='settings'){cr='settings';em=false}else{cr='wiki';em=false;cs=segs.slice(1).join('/')||HOME_PAGE}}
    else{if(segs[0]==='login'){cr='login';em=false}else if(segs[0]==='settings'){cr='settings';em=false}else{cr='wiki';em=false;cs=cl||HOME_PAGE}}
    if(lfp)lang=lfp;const ql=pr.get('lang');if(ql==='en'||ql==='fa')lang=ql;
  }

  $effect(()=>{rr();window.addEventListener('hashchange',rr);return()=>window.removeEventListener('hashchange',rr)});
  function nv(s){window.location.hash=`#/${lang}/${s}`}
  function ru(rt){window.location.hash=`#/${lang}/${rt}`}
  function te(){if(em)window.location.hash=`#/${lang}/${cs}`;else window.location.hash=`#/edit/${lang}/${cs}`}
  function hs(q){sq=q}function ts(){so=!so;ss(so)}function cls(){so=false;ss(false)}function opn(){so=true;ss(true)}
</script>

<div class="al">
  <Header {lang} {user} {role} currentRoute={cr} currentSlug={cs} onToggleSidebar={ts} onSearch={hs} onNavigate={nv} onRoute={ru}/>
  <div class="ab">
    {#if cr==='wiki'||cr==='login'||cr==='settings'}<Sidebar {lang} isOpen={so} currentSlug={cs} onNavigate={nv} onClose={cls} onOpen={opn}/>{/if}
    <main class="mc">
      ...
      {:else if cr==='settings'}{#if user}<SettingsPage {lang} {user} {role}/>{:else}<div class="pe"><h2>{t(lang,'pageNotFound')}</h2><p><a href="#/{lang}/login">{t(lang,'login')}</a></p></div>{/if}
      {:else if em&&user}<LocalEditor slug={cs} {lang}/>
      {:else if em}<div class="pe"><h2>{t(lang,'pageNotFound')}</h2><p><a href="#/{lang}/login">{t(lang,'login')}</a></p></div>
...
<style>.al{display:flex;flex-direction:column;min-height:100vh;background:var(--color-bg-secondary);transition:background var(--transition-normal)}.ab{display:flex;flex:1;max-width:1200px;width:100%;margin:0 auto}.mc{flex:1;min-width:0;display:flex;justify-content:center;position:relative}.pe{display:flex;flex-direction:column;align-items:center;justify-content:center;min-height:50vh;text-align:center;gap:1rem}.pe h2{color:var(--color-text-secondary);margin:0}.fab{position:fixed;bottom:1.5rem;left:1.5rem;z-index:80;width:48px;height:48px;border:none;border-radius:50%;background:var(--color-accent);color:var(--color-accent-text);box-shadow:var(--shadow-md);cursor:pointer;display:flex;align-items:center;justify-content:center;transition:all var(--transition-fast)}.fab:hover{background:var(--color-accent-dark);box-shadow:var(--shadow-lg);transform:scale(1.05)}@media(max-width:768px){.ab{flex-direction:column}}</style>
