...
function t(key) {
  const d = DEFAULT_TRANSLATIONS.fa?.translations || {};
  return d[key] || key;
}
...
<div class="sw">
  <div class="swc">
    ...
    <div class="slb">
      <span class="slbl" class:slbl--a={step >= 1}>{t('stepAdmin')}</span>
      <span class="slbl" class:slbl--a={step >= 2}>{t('stepLang')}</span>
      <span class="slbl" class:slbl--a={step >= 3}>{t('stepReview')}</span>
    </div>
    ...
    <div class="scb">
      ...
      {:else if step === 2}
        <div class="sfg"><label class="sfl" for="sw-dl">{t('defaultLang')}</label><select id="sw-dl" class="sfi" bind:value={form.defaultLanguage}><option value="fa">فارسی (fa)</option><option value="en">English (en)</option></select></div>
        <div class="sfg"><label class="sfl">{t('extraLangs')}</label>
          <div class="sad"><input id="sw-lc" class="sfi swi" type="text" placeholder="fa" bind:value={newLangCode} /><input class="sfi swi" type="text" placeholder="فارسی" bind:value={newLangName} /><button class="sab" onclick={addExtraLang}>+ {t('addLang')}</button></div>
          {#if form.extraLanguages.length > 0}<div class="sel">{#each form.extraLanguages as el}<span class="set">{el.code} - {el.name} <button class="sex" onclick={() => removeExtraLang(el.code)}>x</button></span>{/each}</div>{/if}
        </div>

      {:else}
        <div class="srv"><h3 class="srt">{t('confirmTitle')}</h3><p class="srs">{t('confirmMsg')}</p>
          <div class="src">
            ...
            {#each form.extraLanguages as el}<div class="srr"><span class="srk">{t('langLabel')}</span><span class="srv2">{el.name} ({el.code})</span></div>{/each}
          </div>
        </div>
      {/if}
    </div>
    ...
    <div class="snb">
      {#if step > 1}<button class="snb2" onclick={() => step--}>{t('backToHome')}</button>{/if}
      {#if step < totalSteps}<button class="snp" onclick={() => step++}>Next</button>
      {:else}<button class="snp snp--go" onclick={handleInstall}>{t('install')}</button>{/if}
    </div>
  </div>
</div>
{/if}
...
<style>
  ...
  .swh{text-align:center;margin-bottom:1.5rem}
  ...
  .sid{width:28px;height:28px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:var(--font-size-xs);font-weight:700;border:2px solid var(--color-border);color:var(--color-text-muted);transition:all var(--transition-fast)}
  ...
  .sil{width:40px;height:2px;background:var(--color-border);transition:all var(--transition-fast)}
  .sil--a{background:var(--color-accent)}
  .slb{display:flex;justify-content:space-between;margin-bottom:1.5rem;font-size:var(--font-size-xs);color:var(--color-text-muted);padding:0 0.5rem}
