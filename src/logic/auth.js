...
export async function signIn(email, password) {
  ...
  if (!cfg?.admin?.email || !cfg?.admin?.passwordHash) {
    return { error: 'تنظیمات ادمین یافت نشد' };
  }
  if (email !== cfg.admin.email) {
    return { error: 'ورود ناموفق' };
  }
  const hash = await sha256(password);
  if (hash !== cfg.admin.passwordHash) {
    return { error: 'ورود ناموفق' };
  }
  setStored(email);
  return { user: { email } };
...
export async function signUp() {
  return { error: 'ثبت‌نام در حالت محلی پشتیبانی نمی‌شود' };
}
