...
<div class="sw">
  <div class="swc">
    ...
    <div class="scb">
      {#if error}
        <div class="se2">{error}</div>
      {/if}

      {#if step === 1}
        <div class="sfg">
          <label class="sfl">{t('adminUser')}</label>
          <input class="sfi" type="text" bind:value={form.username} dir="ltr" />
        </div>
        <div class="sfg">
          <label class="sfl">{t('adminPass')}</label>
          <input class="sfi" type="password" bind:value={form.password} placeholder="••••••••" />
        </div>
        <div class="sfg">
          <label class="sfl">{t('adminName')}</label>
          <input class="sfi" type="text" bind:value={form.displayName} />
        </div>

      {:else if step === 2}
        <div class="sfg">
          <label class="sfl">{t('defaultLang')}</label>
          <select class="sfi" bind:value={form.defaultLanguage}>
        ...
        <div class="sfg">
          ...
          <div class="sad">
            <input class="sfi swi" type="text" placeholder="fa" bind:value={newLangCode} />
            <input class="sfi swi" type="text" placeholder="فارسی" bind:value={newLangName} />
            <button class="sab" onclick={addExtraLang}>+ {t('addLang')}</button>
          </div>
          {#if form.extraLanguages.length > 0}
            <div class="sel">
              {#each form.extraLanguages as el}
                <span class="set">{el.code} — {el.name} <button class="sex" onclick={() => removeExtraLang(el.code)}>✕</button></span>
              {/each}
            </div>
          {/if}
        </div>
      ...
      {:else if step === 3}
        <div class="srv">
          ...
          <div class="src">
            ...
            <div class="srr"><span class="srk">{t('mainLangLabel')}</span><span class="srv2">{form.defaultLanguage === 'fa' ? 'فارسی' : 'English'}</span></div>
            {#each form.extraLanguages as el}
    ...
    <div class="snb">
      ...
      {/if}
      {#if step < totalSteps}
        <button class="snp" onclick={() => step++}>{lang === 'en' ? 'Next' : 'بعدی'} →</button>
      {:else}
