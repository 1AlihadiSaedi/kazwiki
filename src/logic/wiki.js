...
export const onPageSaved={_fns:[]};
onPageSaved.listen=fn=>{onPageSaved._fns.push(fn);return()=>{onPageSaved._fns=onPageSaved._fns.filter(l=>l!==fn)}};
...
export function getPageContent(slug,lang){
  ...
  const fp2='/src/wiki-content/'+((typeof window!=='undefined'&&window.__EMERALD_CONFIG__?.defaultLanguage)||'fa')+'/'+slug+'.md';
...
export function getAllSlugs(){const idx=getPageIndex();const s={};for(const p of idx){if(!s[p.slug])s[p.slug]={slug:p.slug,titles:{}};s[p.slug].titles[p.lang]=p.title}for(const key of Object.keys(liveCache)){if(!liveCache[key])continue;const[pl,ps]=key.split('/');if(!idx.find(p=>p.slug===ps&&p.lang===pl)){if(!s[ps])s[ps]={slug:ps,titles:{}};const m=liveCache[key].match(/^#\s+(.+)$/m);s[ps].titles[pl]=m?m[1].trim():ps}}return Object.values(s)}
export async function fetchAllPages(lang){try{const r=await fetch('/api/pages?lang='+(lang||'fa'));if(r.ok)return await r.json()}catch{}return getAllSlugs()}
export function searchPages(query,lang){if(!query||!query.trim())return null;const q=query.toLowerCase().trim();const eq=q.replace(/[.*+?^${}()|[\]\\]/g,'\\$&');const bb='[\\s.,!?;:()\\[\\]{}\\x22\\x27«»،؛؟\\-–—/\\\\]';const re=new RegExp('(^|'+bb+')('+eq+')($|'+bb+')','gi');const r=[];for(const[fp,content]of Object.entries(pages)){if(!content)continue;const m=re.exec(content);re.lastIndex=0;if(!m)continue;const ci=m.index+(m[1]?m[1].length:0);const parts=fp.split('/');const fname=parts.pop();const pl=parts.pop();const slug=fname.replace(/\.md$/,'');const tm=content.match(/^#\s+(.+)$/m);const title=tm?tm[1].trim():slug;if(pl!==lang)continue;const s=Math.max(0,ci-30);const e=Math.min(content.length,ci+q.length+30);let sn=content.substring(s,e).replace(/\n/g,' ').trim();if(s>0)sn='…'+sn;if(e<content.length)sn=sn+'…';r.push({slug,lang:pl,title,snippet:sn})}return r}
